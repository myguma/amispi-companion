...
✅ npm run qa:field-status → correctly blocked release readiness while v1.6.0 version files/annotated local+remote tag / release docs / Release workflow/artifacts / Automated Checks / Day 1-7 2h evidence / Known Issues / gates remain pending
...
- v1.6.0 release判断前: `npm run qa:field-status` を実行し、v1.6.0 version files/annotated local+remote tag / release docs / Release workflow/artifacts / Automated Checks / Day 1-7 2h evidence / Product Gates / Privacy Boundary / Known Issues Day 1-7 / critical issue のpendingが残っていないことを確認する
