...
## 1. Current State (v0.1.25)

### 実装済み

- Tauri v2 + React 18 + TypeScript + Vite + Rust
...
- SpeechBubble / TinyWhisper
- 設定ウィンドウ (Transparency / Behavior)
- Privacy Gate / Permission Level 0-4
- localStorage MemoryEvent log (500件)
- AIProvider 抽象 (companion/ai/ に新インターフェース済み)
- RuleProvider / MockProvider
- 自動更新 / タスクトレイ / グローバルショートカット

### 現在の AIProvider の限界

```ts
// 旧: systems/ai/AIProvider.ts (useCompanionState が呼んでいる)
getAIResponse(recentEvents: MemoryEvent[], userInput?: string): Promise<string>

// 新: companion/ai/ (より良い設計だが useCompanionState に繋がっていない)
respond(input: AIProviderInput): Promise<AIProviderOutput>
```

`requestAIResponse` が旧インターフェースを呼んでおり、ObservationSnapshot を使っていない。
