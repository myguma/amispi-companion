...
現在のQA通過状態は以下で確認できる。このコマンドはread-onlyで、v1.6.0 version files/annotated tag、release docs、Release workflow/artifacts、Automated Checks、Day 1〜7、日次記録の `Day N判定` と2時間以上の実行時間、Product Gates、Privacy Boundary Regression Checks、Known Issues Day 1〜7 / critical issue の pending を検出する。
